...
- Location: tests\orderConfirmation.spec.js:9:1

# Error details

```
Error: toHaveCount can be only used with Locator object
```
...
```ts
  ...
  8  | 
  9  | test('Order checkout flow', async ({ page }) => {
  ...
  14 | 
  15 |     await login.goto();
  16 |     await login.login(userData.users.standard, userData.password);
  17 | 
  18 |     await menu.resetAppState();
  19 | 
  20 |     await inventory.addFirstThreeItemsToCart();
  21 |     await inventory.gotoCart();
  22 | 
> 23 |     await expect(cart.getItems()).toHaveCount(3);
     |                                   ^ Error: toHaveCount can be only used with Locator object
  24 | })
```