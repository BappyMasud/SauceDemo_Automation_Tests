...
Call log:
  - waiting for locator('[data-test="product_sort_container"]') to be visible
...
```ts
  ...
  15 | 
  16 |     async sortByZToA() {
> 17 |         await this.page.waitForSelector('[data-test="product_sort_container"]');
     |                         ^ Error: page.waitForSelector: Test timeout of 30000ms exceeded.
  18 |         await this.page.selectOption('[data-test="product_sort_container"]', 'Name (Z to A)');
  19 |     }
  20 | 
  21 |     async addFirstitem() {
  22 |         const name = await this.page.locator('.inventory_item_name').first().textContent();
  23 |         await this.page.locator('.inventory_item_name').first().click();
  24 |         return name;
  25 |     }
  26 | }
  27 | 
  28 | module.exports = InventoryPage;
```